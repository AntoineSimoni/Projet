...
def le_jeu():
    def gametick():
        ...
        def victoire_j2():
            global score_j1,score_j2
            victoire = Tk()
            lab=Label(victoire, text="Le joueur 2 à gagné "+str(score_j2)+str(" ")+" à "+str(score_j1)+"en")
            lab.grid(row=1,column=2)
            score_j1,score_j2=0,0
            end_menu_rejouer = Button(victoire,text="Rejouer",command=lambda:jouer2())
            end_menu_rejouer.grid(row=2,column=2)
            def jouer2():
                victoire.destroy()
                le_jeu()
            end_menu_menu = Button(victoire,text="Menu",command=lambda:retour_menu())
            end_menu_menu.grid(row=2,column=1)
            def retour_menu():
                victoire.destroy()
                menu_principal()
            victoire.mainloop()
    ...
    j1 = canvas.create_rectangle((15,300),(30,420),fill=couleurs[coul_j1%5])
    j2 = canvas.create_rectangle((1065,300),(1050,420),fill=couleurs[coul_j2%5])
